...
window.addEventListener("load", () => {
  ...
  socket.onmessage = (event) => {
      ...
      file_name = event_parsed.audio;
      console.log("---->" , {event_parsed} , `File : ${file_name}` , "CLIENT >>>>>>>>" , {socket})
...
//   const socket = io( websocketUrl ,  (options = { transports: ["websocket"] }));
...
//     console.log("client: received audio stream");
//     // playAudio(data);
//     playAudio(data.data);
//   });


//   socket.on("audio-stream-file" , (data)=>{
//     console.log("client: received audio file");
//     playAudioFile(data.data)
//   })

//   socket.on("transcript", (transcript) => {
//     if (transcript !== "")
//       captions.innerHTML = transcript ? `<span>You : ${transcript}</span>` : "";
//   });

//   socket.on("llm-resp", (responce_llm) => {
//     if (responce_llm !== "")
//       llm_captions.innerHTML = responce_llm ? `<span>AI : ${responce_llm}</span>` : "";
//   });
