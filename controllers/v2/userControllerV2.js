...
const followUnFollowUserV2 = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json(await followUnFollowServiceV2(currentUser, targetUserId));
  } catch (err) {
    // console.log({err})
    const { status } = err;
...
const updateUserV2 = async (req, res) => {
  try {
    const { full_name, password, username, bio, age, profilePic } = req.body;
...
export { followUnFollowUserV2, updateUserV2 };
